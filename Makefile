...
#* Docker variables
IMAGE := bot_especialist
VERSION := $(shell poetry version --short)
TAG := $(shell echo "latest")

#* Wheel Path
WHEEL_PATH := src/bot_especialist/
...
create-venv:
	...
	@printf "[Makefile] - Ensuring 3.9 is installed...\n"
	@if ! brew list python@3.9 > /dev/null 2>&1; then \
		printf "[Makefile] - 3.9 not found. Installing 3.9 via Homebrew...\n"; \
		brew install python@3.9; \
	else \
		printf "[Makefile] - 3.9 is already installed!\n"; \
	fi
	@printf "[Makefile] - Linking 3.9 to be the default Python...\n"
	@brew link --overwrite python@3.9
	@printf "[Makefile] - Creating a virtual environment named 'bot-especialist-3.9'...\n"
	@if [ ! -d "bot-especialist-3.9" ]; then \
		python3.9 -m venv bot-especialist-3.9; \
	else \
		printf "[Makefile] - Virtual environment 'bot-especialist-3.9' already exists!\n"; \
	fi
	@printf "[Makefile] - Virtual environment setup complete. To activate, run:\n"
	@printf "[Makefile] - 'source bot-especialist-3.9/bin/activate'\n"

#* Activate the virtual environment
.PHONY: activate-venv
activate-venv:
	@printf "[Makefile] - Activating the virtual environment 'bot-especialist-3.9'...\n"
	@source bot-especialist-3.9/bin/activate && printf "[Makefile] - Virtual environment activated!\n"
...
.PHONY: start-airflow
start-airflow:
	@printf "[Makefile] - Setting up Airflow home to 'src/bot_especialist'.\n"
	@AIRFLOW_HOME=$(shell pwd)/src/bot_especialist \
...
start-jupyterlab:
	...
	@chmod +x src/bot_especialist/experiments/start_jupyterlab.sh
	@if [ ! -f src/bot_especialist/experiments/start_jupyterlab.sh ]; then \
		printf "[Makefile] - start_jupyterlab.sh not found!\n"; \
		exit 1; \
	fi
	@src/bot_especialist/experiments/start_jupyterlab.sh
...
setup-env:
	...
	COMPOSER_BUCKET="gs://bot_especialist"; \
	DOCKER_IMAGE_TAG="$(IMAGE):$(VERSION)-$${SANITIZED_TAG}"; \
	DOCKER_FILE_TAG="docker_version.txt"; \
	ARTIFACT_REGISTRY_URI="us-central1-docker.pkg.dev/bot_especialist-dev"; \
...
sync-files:
	...
	# Sync bot_especialist folder
	gsutil -m rsync -x 'endpoints/.*|experiments/.*|dags/.*dag\.py' -r src/bot_especialist $(COMPOSER_BUCKET)/dags/bot_especialist
