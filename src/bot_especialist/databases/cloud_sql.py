...
class CloudSQL:
    ...
    def __get_connection(self) -> Engine:
        ...
        engine = sqlalchemy.create_engine(
            f"postgresql+pg8000://{self.ip_address}",
            creator=getconn,
        )
        return engine
    ...
    def __session_scope(self) -> Connection:
        ...
        logging.info("Initialized CloudSQL connection.")
        try:
            yield connection
            transaction.commit()
            ...
            logging.warning(
                f"Rolled back the transaction on CloudSQL due to the error: {err}."
            )
            raise err
        finally:
            connection.close()
            self.connector.close()
            logging.info("Closed connection from CloudSQL.")
    ...
    def run_query(self, query: str) -> List[Tuple]:
        try:
            with self.__session_scope() as session:
                ...
                logging.info("Executed query successfully on CloudSQL.")
                rows = result.fetchall()  # Fetch the rows
                return rows
        except Exception as err:
            logging.error(f"Failed to run query on CloudSQL: \n\n{err}\n\n")
            raise err


# Usage example:
if __name__ == "__main__":
    configs = read_yaml("app-config.yml")
